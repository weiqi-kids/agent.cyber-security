...
若來源為 `docs/Narrator/` 或內容包含資安特徵，需額外偵測：

| 特徵 | 報告類型 | Schema |
|------|----------|--------|
| CVE 編號、CVSS 評分 | 技術分析 | `TechArticle` |
| 週報、定期彙整、統計 | 威脅週報 | `NewsArticle` + `DataFeed` |
| 防禦建議、修補指南 | 防禦建議 | `TechArticle` |

```json
{
  "security_report_detection": {
    "is_security_report": true,
    "report_type": "threat_weekly",
    "article_schema": "NewsArticle",
    "additional_schemas": ["DataFeed"],
    "reason": "包含週報統計、CVE 彙整、定期發布特徵"
  }
}
```
