...
> 本區塊定義資訊安全威脅報告、週報等專業內容的 SEO 規則。

### 資安報告 Article 類型

資安報告應使用 `TechArticle` 或 `NewsArticle`，取代通用 `Article`：

| 報告類型 | Schema Type | 適用情境 |
|----------|-------------|----------|
| 威脅週報 | `NewsArticle` | 定期發布、時效性內容 |
| 技術分析 | `TechArticle` | 深度技術內容、漏洞分析 |
| 防禦建議 | `TechArticle` | 操作指南、最佳實踐 |
...
**TechArticle 專屬欄位**：
- `proficiencyLevel`：技術難度（`Beginner`、`Intermediate`、`Advanced`、`Expert`）
- `dependencies`：閱讀前提（如「熟悉 CVE 編號系統」）
...
```json
{
  ...
  "inLanguage": "zh-TW",
  "isAccessibleForFree": true,
  "dateline": "{{REPORT_PERIOD}}"
}
```

**NewsArticle 專屬欄位**：
- `dateline`：報導時段（如「2026 年第 7 週」）

---

### 資安報告專用 Schema

#### 18. SecurityAdvisory（安全公告）
...
{
  "@type": "TechArticle",
  "@id": "{{CANONICAL_URL}}#security-advisory",
  "about": {
  ...
  "headline": "{{CVE_ID}}: {{VULNERABILITY_TITLE}}",
...
```html
<div class="threat-summary speakable-content">
  <h3>本週威脅概覽</h3>
  <ul class="threat-highlights">
    <li>Critical 漏洞：3 個</li>
    <li>主動利用中：2 個</li>
    <li>需立即修補：CVE-2026-XXXX</li>
  </ul>
</div>
```
...
```html
<div class="vulnerability-impact key-answer" data-question="CVE-2026-XXXX 影響範圍">
  <strong>影響產品</strong>：Windows Server 2022、Windows 11
  <strong>CVSS 評分</strong>：9.8（Critical）
  <strong>建議行動</strong>：立即套用安全更新
</div>
```
...
```html
<ol class="actionable-steps defense-recommendations">
  <li>確認受影響系統版本</li>
  <li>測試並部署安全更新</li>
  <li>監控異常網路活動</li>
  <li>檢視存取日誌</li>
...
| 優先級 | 來源類型 | 範例 |
|--------|----------|------|
| **1** | CVE/NVD | nvd.nist.gov、cve.org |
| **2** | CERT/CSIRT | CISA、TWCERT、JPCERT |
| **3** | 廠商安全公告 | Microsoft Security、Google Security Blog |
| **4** | 研究機構 | MITRE ATT&CK、SANS |
...
```html
<div class="disclaimer security-disclaimer">
  <p>本報告內容基於公開來源情報彙整分析，僅供資訊安全專業人員參考。</p>
  <p>具體安全決策請依據組織的風險評估與資安政策執行。</p>
  <p>報告中提及的漏洞資訊可能隨時更新，建議定期查閱原始來源。</p>
</div>
```
...
| # | 檢查項目 | 狀態 |
|---|----------|------|
| 11.1 | 使用 TechArticle 或 NewsArticle（非通用 Article） | ✅/❌ |
| 11.2 | CVE 編號有連結到 NVD 或原始公告 | ✅/❌ |
| 11.3 | CVSS 評分標註（若有） | ✅/❌ |
| 11.4 | 受影響產品/版本列表 | ✅/❌ |
| 11.5 | 防禦建議使用 `.actionable-steps` 標記 | ✅/❌ |
| 11.6 | 威脅摘要使用 `.speakable-content` 標記 | ✅/❌ |
| 11.7 | 資安免責聲明存在 | ✅/❌ |
| 11.8 | 權威來源連結（CVE/CERT/廠商）≥ 2 個 | ✅/❌ |
