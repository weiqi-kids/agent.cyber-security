...
> 適用條件：來源為 `docs/Narrator/` 或 Writer 判斷為資安報告

| # | 檢查項目 | 狀態 | 問題描述 |
|---|----------|------|----------|
| 8.1 | 使用 TechArticle 或 NewsArticle（非通用 Article） | ✅/❌/N/A | |
| 8.2 | CVE 編號有連結到 NVD 或原始公告 | ✅/❌/N/A | |
| 8.3 | CVSS 評分標註（若內容有提及） | ✅/❌/N/A | |
| 8.4 | 受影響產品/版本列表（若內容有提及） | ✅/❌/N/A | |
| 8.5 | 防禦建議使用 `.actionable-steps` 標記 | ✅/❌/N/A | |
| 8.6 | 威脅摘要使用 `.speakable-content` 標記 | ✅/❌/N/A | |
| 8.7 | 資安免責聲明存在 | ✅/❌/N/A | |
| 8.8 | 權威來源連結（CVE/CERT/廠商）≥ 2 個 | ✅/❌/N/A | |
| 8.9 | 週報使用 DataFeed Schema（若為定期報告） | ✅/❌/N/A | |
| 8.10 | proficiencyLevel 欄位（TechArticle） | ✅/❌/N/A | |

#### 資安報告特殊規則

1. **Article 類型判斷**：
   - 週報/定期彙整 → `NewsArticle`
   - 技術分析/漏洞詳解 → `TechArticle`
   - 使用通用 `Article` 視為錯誤

2. **CVE 連結檢查**：
   - 每個 CVE 編號必須是可點擊連結
   - 優先連結 NVD（nvd.nist.gov）
   - 次優先連結 cve.org
...
**資安報告專用（若適用）**：
- [ ] TechArticle 或 NewsArticle（非通用 Article）
- [ ] CVE 連結到 NVD/原始公告
- [ ] DataFeed（週報類）
- [ ] `.actionable-steps`（防禦建議）
- [ ] `.speakable-content`（威脅摘要）
- [ ] 資安免責聲明
- [ ] 權威來源連結 ≥ 2 個
