...
CATEGORIES=(attack_incident vulnerability_disclosure policy_regulation industry_trend other)
...
extract_embed_text() {
  ...
  # 萃取純文字（標題 + 摘要 + 事實內容 + 受影響系統）
  echo "$title $(echo "$content" | awk '
    /^## (摘要|事實內容|受影響系統)/ { cap=1; next }
    /^## / { cap=0; next }
    /^---/ { cap=0; next }
    cap && /^\| / { next }
    cap { print }
...
build_payload() {
  ...
  jq -nc \
    ...
    --arg severity "$(echo "$content" | grep '| \*\*嚴重程度\*\*' | sed -E 's/.*\| *\*\*嚴重程度\*\* *\| *//;s/ *\|$//' | head -1)" \
