...
```
docs/Extractor/security_news_facts/{category}/{YYYY}-{MM}-{DD}-{唯一識別符}.md
```

#### 檔名生成規則（嚴格優先順序）

1. **有明確事件代號**：`{日期}-{事件代號}.md`
   - 範例：`2026-02-15-apt-c-60-update.md`
   - 事件代號通常來自威脅行為者名稱、行動代號等

2. **一般新聞**：`{日期}-{標準化slug}.md`
   - 範例：`2026-02-15-ivanti-connect-secure-malware.md`
   - slug 標準化規則：
     - 全小寫
     - 空格和特殊字元轉為 `-`
     - 保留產品名稱、廠商名稱等關鍵識別資訊
...
3. **重複處理**：若檔案已存在（透過索引檢查）
   - 若相同日期 + 相同主題已存在 → **跳過**，輸出「SKIP: 已存在於 {路徑}」
   - 若為同一事件的後續更新 → 加日期區分或序號 `{原檔名}-update.md`

> **強制規則**：萃取 Task 必須先檢查傳入的索引，避免產生重複檔案。檔名必須包含完整日期（YYYY-MM-DD）以便追蹤時序。
...
- [ ] 分類是否準確？（符合分類規則表）
- [ ] 嚴重程度判定是否有依據？（非主觀臆測）
- [ ] **檔名是否符合標準化規則？**（包含完整日期 YYYY-MM-DD、slug 標準化）
- [ ] **是否已檢查索引確認無重複？**（跳過已存在的相同事件）
