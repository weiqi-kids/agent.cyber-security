# Domain names are actively marketed to fi-domain name holders

| 項目 | 內容 |
|------|------|
| **日期** | 2020-07-28 |
| **來源** | NCSC-FI (Finland) |
| **分類** | other |
| **嚴重程度** | 未評估 |
| **信心水準** | 高 |

## 摘要

Domain names are marketed actively and, at regular intervals, even aggressively. It is worthwhile to consider in advance which domain names you need or want for yourself or for your company. The easiest way is to register all of the domain names of your choice using the same registrar. On...

## 事實內容

Domain names are marketed actively and, at regular intervals, even aggressively. It is worthwhile to consider in advance which domain names you need or want for yourself or for your company. The easiest way is to register all of the domain names of your choice using the same registrar. On the internet, several hundreds of different domain names can be used, and it is not by any means practical to register all of them for yourself.

## 受影響系統

- (To be determined from full content)

## 來源連結

- [Domain names are actively marketed to fi-domain name holders](https://www.kyberturvallisuuskeskus.fi/en/news/domain-names-are-actively-marketed-fi-domain-name-holders)

---
> 萃取時間:2026-01-27 05:22 UTC
> 資料來源:NCSC-FI (Finland) ncsc-fi
