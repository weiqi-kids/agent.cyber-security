# Threat level in cyber environment has risen – activity towards Finland has increased

| 項目 | 內容 |
|------|------|
| **日期** | 2022-09-15 |
| **來源** | NCSC-FI Security Now! |
| **分類** | attack_incident |
| **嚴重程度** | 未評估 |
| **信心水準** | 高 |

## 摘要

The number of cyberattacks has increased globally this year. After a quieter period in the spring, the number of attacks is on the rise in Finland. Based on reports received by the National Cyber Secu...

## 事實內容

The number of cyberattacks has increased globally this year. After a quieter period in the spring, the number of attacks is on the rise in Finland. Based on reports received by the National Cyber Security Centre Finland (NCSC-FI) at Traficom, attacks against Finnish organisations increasingly often employ malware, phishing and denial-of-service attacks.

## 受影響系統

- N/A (需進一步分析)

## 來源連結

- [Threat level in cyber environment has risen – activity towards Finland has increased](https://www.kyberturvallisuuskeskus.fi/en/news/threat-level-cyber-environment-has-risen-activity-towards-finland-has-increased)

---
> 萃取時間：2026-02-17 08:10 UTC
> 資料來源：NCSC-FI Security Now! (ncsc-fi-secnow)

