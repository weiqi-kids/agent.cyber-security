# QR codes are used more often in phishing

| 項目 | 內容 |
|------|------|
| **日期** | 2023-07-11 |
| **來源** | NCSC-FI (Finland) |
| **分類** | attack_incident |
| **嚴重程度** | 未評估 |
| **信心水準** | 高 |

## 摘要

‘Quishing’ is phishing carried out using a QR code. The victim is lured to scan a QR code with their smartphone or tablet. QR codes are easy to open using a smartphone or tablet due to their built-in ...

## 事實內容

‘Quishing’ is phishing carried out using a QR code. The victim is lured to scan a QR code with their smartphone or tablet. QR codes are easy to open using a smartphone or tablet due to their built-in camera made to recognise a QR code.

## 受影響系統

- 無特定系統

## 來源連結

- [QR codes are used more often in phishing](https://www.kyberturvallisuuskeskus.fi/en/news/qr-codes-are-used-more-often-phishing)

---
> 萃取時間: 2026-02-20 00:46 UTC
> 資料來源: NCSC-FI (Finland) ncsc-fi-secnow
