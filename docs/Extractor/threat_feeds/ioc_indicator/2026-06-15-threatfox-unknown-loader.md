# IoC 指標：Unknown Loader

| 項目 | 內容 |
|------|------|
| **日期** | 2026-06-15 |
| **來源** | abuse.ch ThreatFox |
| **分類** | ioc_indicator |
| **威脅類型** | payload_delivery |
| **惡意程式家族** | Unknown Loader |
| **狀態** | unknown |
| **威脅等級** | 未評估 |
| **信心水準** | 中 |

## 威脅描述

abuse.ch ThreatFox 分享的 IoC 指標（威脅類型：payload_delivery；惡意程式：Unknown Loader）。信心水準：50。最後出現：N/A。

## IoC 指標

| 類型 | 值 |
|------|-----|
| url | `hxxps://lbs-lamb-withdrawal-thickness.trycloudflare.com/` |

## 關聯分析

無已識別的關聯。

## 來源連結

- [abuse.ch ThreatFox](https://threatfox.abuse.ch/)

---
> 萃取時間：2026-06-17 15:40 UTC
> 資料來源：abuse.ch ThreatFox threatfox-recent
