...
| 優化項目 | 措施 | 預期效果 |
|----------|------|----------|
| 搜尋索引精簡 | 將 `docs/Extractor/`（~1,800 頁）排除在搜尋索引外 | 減少 Lunr.js 索引建置時間 |
| JavaScript 延遲執行 | seo-report.html 使用 `requestIdleCallback`，head_custom.html 使用 `DOMContentLoaded` | 不阻塞主執行緒 |

> **待驗證**：需重新執行 Lighthouse 測試確認效能改善程度。
...
| 防禦建議 | /docs/Narrator/defense_advisory/ | 索引頁 | ✅ | P0 |
| 資料來源 | /sources/ | 說明頁 | ✅ | P1 |
...
> **更正**：防禦建議頁面（/docs/Narrator/defense_advisory/）正常運作。
